...
// Replicating the ChatMessageBubble component
function ChatMessageBubble({ message, aiEmoji, sources }: { 
  message: Message, 
  aiEmoji?: string,
  sources?: any[]
}) {
  ...
  return (
    <div className={`flex ${isAiMessage ? "justify-start" : "justify-end"} mb-4`}>
      <div className={`flex ${isAiMessage ? "bg-gray-800" : "bg-blue-600"} p-4 rounded-lg max-w-[80%]`}>
        {isAiMessage && aiEmoji && <div className="mr-2">{aiEmoji}</div>}
...
// Replicating the IntermediateStep component
function IntermediateStep({ message }: { message: Message }) {
  ...
  return (
    <div className="flex mb-4">
      <div className="bg-gray-700 p-4 rounded-lg max-w-[80%] text-sm">
...
export default function StandaloneChatPage() {
  ...
  const emoji = "🌌"
  const titleText = "Nova AI"
  const placeholder = "Speak your cosmic truth..."
  const emptyStateComponent = <div className="text-[#e0e0ff]">Ready to begin your cosmic journey...</div>
  const showIntermediateStepsToggle = true
